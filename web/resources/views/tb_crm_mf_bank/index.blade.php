...
<div class="container">
    ...
    <div class="card shadow mt-3 mb-3">
        <div class="card-header">
            <a href="{{ route('banks.index') }}" class="text-black text-decoration-none">BANKS</a>
        </div>
        <div class ="card-body">
                ...
                <tbody>
                    @forelse ($banks as $bank)
                        <tr>
                            ...
                            <td><a href="{{ route('banks.show', ['bank' => $bank->id]) }}">{{ sprintf('%08d', $bank->id) }}</a></td>
                            <td>{{ $bank->code }}</td>
                            <td>{{ $bank->name }}</td>
                            <td><input class="form-check-input" type="checkbox" value="1" {{ ($bank->is_active??0) == 1 ? "checked":""}} disabled></td>
                            <td>
                                <div class="form d-inline">
                                    <a href="{{ route('banks.edit', ['bank' => $bank->id]) }}" class="btn btn-sm btn-secondary action-btn">
                                        <i class="fa-solid fa-pencil"></i>
                                    </a>
                                </div>
                                <form method="POST" class="form d-inline "
                                    action="{{ route('banks.destroy', ['bank' => $bank->id]) }} " class="d-inline">
                                    @csrf
                                    @method('DELETE')
