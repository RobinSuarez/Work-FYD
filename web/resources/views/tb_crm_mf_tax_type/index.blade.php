...
<div class="container">
    <div class="card shadow">
        ...
        <div class="card-body">
            <form method="GET" action="{{route('tax-types.index')}}" >
                @csrf
    ...
    <div class="card shadow mt-3 mb-3">
        <div class="card-header">
            <a href="{{ route('tax-types.index') }}" class="text-black text-decoration-none">TAX TYPES</a>
        </div>
        <div class ="card-body">
                ...
                <tbody>
                    @forelse ($tax_types as $tax_type)
                        <tr>
                            ...
                            <td><a href="{{ route('tax-types.show', ['tax_type' => $tax_type->id]) }}">{{ sprintf('%08d', $tax_type->id) }}</a></td>
                            <td>{{ $tax_type->code }}</td>
                            <td>{{ $tax_type->name }}</td>
                            <td><input class="form-check-input" type="checkbox" value="1" {{ ($tax_type->is_active??0) == 1 ? "checked":""}} disabled></td>
                            <td>
                                <div class="form d-inline">
                                    <a href="{{ route('tax-types.edit', ['tax_type' => $tax_type->id]) }}" class="btn btn-sm btn-secondary action-btn">
                                        <i class="fa-solid fa-pencil"></i>
                                    </a>
                                </div>
                                <form method="POST" class="form d-inline "
                                    action="{{ route('tax-types.destroy', ['tax_type' => $tax_type->id]) }} " class="d-inline">
                                    @csrf
                                    @method('DELETE')
